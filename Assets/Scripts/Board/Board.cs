...
public class Board
{
    ...
    internal void Fill()
    {
        NormalItem.eNormalType[] allTypes = new NormalItem.eNormalType[]
        {
        ...
        List<NormalItem.eNormalType> typesToAssign = new List<NormalItem.eNormalType>();

        int baseCount = 3; // mỗi con 3 ô
        int remainder = totalCells - baseCount * allTypes.Length;

        // thêm cho con đầu tiên phần dư
        typesToAssign.AddRange(Enumerable.Repeat(allTypes[0], baseCount + remainder));

        // thêm các con còn lại
        for (int i = 1; i < allTypes.Length; i++)
        {
            typesToAssign.AddRange(Enumerable.Repeat(allTypes[i], baseCount));
        }

        // shuffle ngẫu nhiên
        typesToAssign = typesToAssign.OrderBy(x => UnityEngine.Random.value).ToList();
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                if (index >= typesToAssign.Count) break;

                Cell cell = m_cells[x, y];
                NormalItem item = new NormalItem();

                item.SetType(typesToAssign[index]);
                index++;
    ...
    public List<Cell> GetAllCells()
    {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                if (m_cells[x, y] != null) list.Add(m_cells[x, y]);
            }
        }
        return list;
    }
